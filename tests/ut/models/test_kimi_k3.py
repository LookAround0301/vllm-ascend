...
def test_kimi_k3_config_matches_upstream_image_and_vision_contract():
    config = KimiK3Config(
    ...
    assert config.image_placeholder == "<|kimi_image_placeholder|>"
    assert not hasattr(config, "use_unified_vision_chunk")
    assert config.vision_config.num_attention_heads == 12
    assert config.vision_config.num_hidden_layers == 7
    assert config.vision_config.hidden_size == 1024
    assert config.vision_config.intermediate_size == 3584
    assert config.vision_config.video_attn_type == "spatial_temporal"
