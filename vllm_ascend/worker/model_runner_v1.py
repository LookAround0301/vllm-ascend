...
class NPUModelRunner(LoRAModelRunnerMixin):
    ...
    def _prepare_inputs(
        ...
        if self.speculative_config and self.pcp_size > 1 and is_prefill:
            self._generate_pcp_mtp_input(num_reqs, scheduler_output.total_num_scheduled_tokens, scheduler_output.num_scheduled_tokens)
        # Copy the tensors to the NPU.
        ...
        return (attn_metadata, positions, num_scheduled_tokens,
                ...
                max_num_scheduled_tokens, long_seq_metadata)
    ...
    def propose_draft_token_ids(
        ...
        input_ids_pcp_full: torch.Tensor = None,
        query_start_loc_pcp_full: torch.Tensor = None,
        is_prefill: bool = False,
        req_scheduled_tokens: List = None,
        ...
        if not self.drafter:
            ...
            draft_token_ids = self.drafter.generate_token_ids(
                ...
                hidden_states, attn_metadata, aux_hidden_states, input_ids_pcp_full, 
                query_start_loc_pcp_full, is_prefill, req_scheduled_tokens, long_seq_metadata)
    ...
    def execute_model(
        ...
        with ProfileExecuteDuration().capture_async("prepare input"):
            ...
            (attn_metadata, positions, num_scheduled_tokens_np,
             ...
             intermediate_tensors,
             max_query_len, long_seq_metadata) = (self._prepare_inputs(scheduler_output,
                                                    intermediate_tensors))
        ...
        with ProfileExecuteDuration().capture_async("post process"):
            ...
            if self.speculative_config:
                self._draft_token_ids = self.propose_draft_token_ids(
                    ...
                    attn_metadata,
                    aux_hidden_states,
                    self.input_ids_pcp_full if self.pcp_size > 1 else None,
                    self.query_start_loc_pcp_full if self.pcp_size > 1 else None,
                    is_prefill,
    ...
    def _generate_pcp_mtp_input(
        ...
        req_indices_pcp_full = np.repeat(self.arange_np[:num_reqs], num_scheduled_tokens_pcp_full)
        ...
        cumsums_offsets_pcp_full = np.repeat(cu_num_tokens_pcp_full - num_scheduled_tokens_pcp_full,
                                    num_scheduled_tokens_pcp_full)
        arange_pcp_full = self.arange_np[:total_num_scheduled_tokens_pcp_full] - cumsums_offsets_pcp_full
        positions_np_pcp_full = self.positions_np_pcp_full[:total_num_scheduled_tokens_pcp_full]
        np.add(self.input_batch.num_computed_tokens_cpu[req_indices_pcp_full],
            arange_pcp_full,
            out=positions_np_pcp_full)
        token_indices_pcp_full = (positions_np_pcp_full +
