...
class NPUModelRunner(LoRAModelRunnerMixin, ECConnectorModelRunnerMixin):
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        if self.speculative_config and self.pcp_size * self.dcp_size > 1:
            ...
            self.query_lens_pcp_full = torch.zeros(self.max_num_reqs,
                                                   dtype=torch.int32,
                                                   device=self.device)
            self.query_lens_pcp_full_cpu = torch.zeros(self.max_num_reqs,
                                                       dtype=torch.int32,
                                                       device="cpu",
                                                       pin_memory=True)
    ...
    def _prepare_inputs(
        ...
        num_valid_tokens = np.array([
            ...
            for num_tokens, i in zip((tokens_before_update
                                      if self.pcp_size > 1 else tokens), req_ids)
        ],
                                    dtype=np.int32)
        ...
        for kv_cache_group_id, kv_cache_group_spec in enumerate(
            ...
            if self.speculative_config and self.pcp_size * self.dcp_size > 1:
                ...
                ori_query_lens_cpu = self.query_lens_pcp_full_cpu[:num_reqs]
                ori_query_lens = self.query_lens_pcp_full[:num_reqs]
                num_prefill_reqs = (ori_query_lens_cpu
                ...
                common_attn_metadata.prefill_context_parallel_metadata.query_lens_pcp_full_cpu = ori_query_lens_cpu
                common_attn_metadata.prefill_context_parallel_metadata.max_query_len_pcp_full = ori_query_lens_cpu.max(
                ).item()
    ...
    def _generate_pcp_metadata(self, total_num_scheduled_tokens):
        ...
        query_lens = self.query_lens_pcp_full_cpu[:num_reqs] \
        ...
        if self.pcp_size * self.dcp_size > 1:
            ...
            if self.pcp_size > 1:
                ...
                long_seq_metadata.pcp_prefill_mask = self.extra_long_seq_kwargs[
                    'pcp_prefill_mask']
                long_seq_metadata.split_q_head_nomask_idx_tensor_list = split_q_head_nomask_idx_tensor_list
                long_seq_metadata.split_q_tail_nomask_idx_tensor_list = split_q_tail_nomask_idx_tensor_list
                long_seq_metadata.head_attn_nomask_seqlens_list = head_attn_nomask_seqlens_list
                long_seq_metadata.tail_attn_nomask_seqlens_list = tail_attn_nomask_seqlens_list
    ...
    def _generate_pcp_mtp_input(
        ...
        self.query_lens_pcp_full_cpu[:num_reqs] = torch.from_numpy(
        ...
        self.query_lens_pcp_full[:num_reqs].copy_(
            self.query_lens_pcp_full_cpu[:num_reqs],
            non_blocking=True,
        )
        ...
        if self.decode_threshold > 2 and not with_prefill:
            ...
            mtp_slot_ori = self.input_batch.block_table.block_tables[
                0].slot_mapping_cpu[:num_tokens_mtp]
